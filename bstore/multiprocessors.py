    ...
    def __call__(self, img, coords):
        ...
        for ctr, currCoords in enumerate(coords):
            ...
            try:
                ...
                if self.bgCorrect:
                    assert self.bgMaskSize > 2 * \
                        self.spotMaskRadius, ('' 'Error: bgMaskSize is less than half spotMaskRadius')
            ...
            photons[ctr] = np.sum(
                img[spotMask] - self.aduOffset - background[ctr]) / self.cameraGain
...
class OverlayClusters:
    ...
    def _initCanvas(self, locs, wfImage, stats):
        ...
        if self.filterCol is None:
            ...
            plotx = (stats.loc[stats[filterCol], xc] - self.xShift) \
                / self.pixelSize
            ploty = (stats.loc[stats[filterCol], yc] - self.yShift) \
                / self.pixelSize
